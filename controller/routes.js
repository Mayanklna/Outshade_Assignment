...
router.get('/eventform', (req, res) => {
    res.render("eventform", { csrfToken: req.csrfToken() });
});
...
router.post('/eventform', (req, res) => {
    // get all the values 
    const {  eventname,venue,guest } = req.body;
    // check if the are empty 
    if (!eventname || !venue || !guest  ) {
        res.render("eventform", { err: "All Fields Required !", csrfToken: req.csrfToken() });
    }  else {

        // validate email and username and password 
        // skipping validation
        // check if a user exists
        user.findOne({ $or: [{ eventname: eventname }, { guest: guest }] }, function (err, data) {
            if (err) throw err;
            if (data) {
                res.render("eventform", { err: "User Exists, Try Logging In !", csrfToken: req.csrfToken() });
            } else {
                // generate a salt
                bcryptjs.genSalt(12, (err, salt) => {
                    if (err) throw err;
                    // hash the password
                    bcryptjs.hash(eventname, salt, (err, hash) => {
                        if (err) throw err;
                        // save user in db
                        event({
                            eventname:   eventname,
